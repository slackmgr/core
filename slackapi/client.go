...
func New(logger Logger, cache Cache, metrics Metrics, options *Options) Client {
	options.SetDefaults()
...
func (c *client) Connect(ctx context.Context) (*slack.AuthTestResponse, error) {
	...
	if c.options.CachePrefix == "" {
		c.options.CachePrefix = "sudo-go-slackapi"
	}
	...
	c.api = slack.New(
		c.options.BotToken,
		slack.OptionDebug(c.options.DebugMode),
		slack.OptionAppLevelToken(c.options.AppToken),
...
func (c *client) ChatPostMessage(ctx context.Context, channelID string, options ...slack.MsgOption) (string, error) {
	...
	ts, _, err := callAPI(ctx, c.logger.WithField("slack_channel_id", channelID), c.options, c.metrics, action, f)
	if err != nil {
		return "", err
	}
...
func (c *client) ChatDeleteMessage(ctx context.Context, channelID string, ts string) error {
	...
	if _, _, err := callAPI(ctx, c.logger.WithField("slack_channel_id", channelID), c.options, c.metrics, action, f); err != nil {
		return err
	}
...
func (c *client) MessageHasReplies(ctx context.Context, channelID, ts string) (bool, error) {
	...
	msgs, _, err := callAPI(ctx, c.logger.WithField("slack_channel_id", channelID), c.options, c.metrics, action, f, ChannelNotFoundError, "thread_not_found")
	if err != nil {
...
func (c *client) GetChannelInfo(ctx context.Context, channelID string) (*slack.Channel, error) {
	...
	cacheKey := fmt.Sprintf("%s-channelInfo-%s", c.options.CachePrefix, channelID)

	cachedValue, cacheHit, err := c.readCacheString(ctx, cacheKey)
	if err != nil {
		return nil, err
	}
	...
	if cacheHit {
		...
		if err := json.Unmarshal([]byte(cachedValue), &info); err != nil {
	...
	channel, _, err := callAPI(ctx, c.logger.WithField("slack_channel_id", channelID), c.options, c.metrics, action, f, ChannelNotFoundError)
	if err != nil {
	...
	if err != nil {
		...
		c.addToCacheWithRandomExpiration(ctx, cacheKey, resultJSON, 2*time.Minute, time.Minute)
	}
...
func (c *client) ListAllChannels(ctx context.Context) ([]*ChannelSummary, error) {
	...
	cacheKey := fmt.Sprintf("%s-listAllChannels", c.options.CachePrefix)
	...
	if cacheHit {
		...
		if err := json.Unmarshal([]byte(cachedValue), &channels); err != nil {
...
func (c *client) ListBotChannels(ctx context.Context) ([]*ChannelSummary, error) {
	...
	cacheKey := fmt.Sprintf("%s-listBotChannel", c.options.CachePrefix)
...
func (c *client) GetUserInfo(ctx context.Context, userID string) (*slack.User, error) {
	...
	cacheKey := fmt.Sprintf("%s-userInfo-%s", c.options.CachePrefix, userID)
	...
	user, _, err := callAPI(ctx, c.logger, c.options, c.metrics, action, f)
	if err != nil {
	...
	if err != nil {
		...
		c.addToCacheWithRandomExpiration(ctx, cacheKey, resultJSON, 10*time.Hour, 5*time.Hour)
	}
...
func (c *client) ListUserGroupMembers(ctx context.Context, groupID string) (map[string]struct{}, error) {
	...
	cacheKey := fmt.Sprintf("%s-userGroupMembers-%s", c.options.CachePrefix, groupID)
	...
	userIDs, _, err := callAPI(ctx, c.logger, c.options, c.metrics, action, f, "no_such_subteam")
	if err != nil {
	...
	if err != nil {
		...
		c.addToCacheWithRandomExpiration(ctx, cacheKey, resultJSON, 3*time.Minute, 2*time.Minute)
	}
...
func (c *client) GetUserByEmail(ctx context.Context, email string) (*slack.User, error) {
	...
	cacheKey := fmt.Sprintf("%s-userByEmail-%s", c.options.CachePrefix, email)
	...
	user, _, err := callAPI(ctx, c.logger, c.options, c.metrics, "users.lookupByEmail", f, "users_not_found")
	if err != nil {
...
func (c *client) ListAllUsers(ctx context.Context) (map[string]*slack.User, error) {
	...
	users, _, err := callAPI(ctx, c.logger, c.options, c.metrics, action, f)
	if err != nil {
...
func (c *client) GetUserIDsInChannel(ctx context.Context, channelID string) (map[string]struct{}, error) {
	...
	cacheKey := fmt.Sprintf("%s-userIdsInChannel-%s", c.options.CachePrefix, channelID)
	...
	for {
		users, nextCursor, err := callAPI(ctx, c.logger.WithField("slack_channel_id", channelID), c.options, c.metrics, action, f, ChannelNotFoundError)
		if err != nil {
			if err.Error() == ChannelNotFoundError {
				return result, nil
			}
	...
	if err != nil {
		...
		c.addToCacheWithRandomExpiration(ctx, cacheKey, resultJSON, time.Minute, time.Minute)
	}
...
func (c *client) BotIsInChannel(ctx context.Context, channelID string) (bool, error) {
	...
	cacheKey := fmt.Sprintf("%s-botIsInChannel-%s", c.options.CachePrefix, channelID)

	cachedValue, cacheHit, err := c.readCacheBool(ctx, cacheKey)
	if err != nil {
		return false, err
	}
	...
	if found {
		c.addToCacheWithRandomExpiration(ctx, cacheKey, found, time.Hour, 30*time.Minute)
	} else {
		c.addToCacheWithRandomExpiration(ctx, cacheKey, found, time.Minute, time.Minute)
	}
...
func (c *client) AddUserToChannel(ctx context.Context, user *slack.User, channelID string) error {
	...
	_, _, err := callAPI(ctx, c.logger.WithField("slack_channel_id", channelID), c.options, c.metrics, action, f)
	if err != nil {
...
func callAPI[V any, W any](ctx context.Context, logger Logger, options *Options, metrics Metrics, action string, f func(ctx context.Context) (V, W, error), expectedErrors ...string) (V, W, error) {
	attempt := 1
	started := time.Now()
	...
	for {
		...
		if waitErr := waitForAPIError(ctx, started, logger, attempt, action, options, err); waitErr != nil {
...
func waitForAPIError(ctx context.Context, started time.Time, logger Logger, attempt int, action string, options *Options, err error) error {
	var rateLimitError *slack.RateLimitedError

	if errors.As(err, &rateLimitError) {
		remainingWaitTime := time.Until(started.Add(options.MaxRateLimitErrorWaitTime))

		if attempt >= options.MaxAttemtsForRateLimitError || remainingWaitTime < time.Second {
	...
	if isTransientError(err) {
		remainingWaitTime := time.Until(started.Add(options.MaxTransientErrorWaitTime))

		if attempt >= options.MaxAttemptsForTransientError || remainingWaitTime < time.Second {
	...
	remainingWaitTime := time.Until(started.Add(options.MaxFatalErrorWaitTime))

	if attempt >= options.MaxAttemptsForFatalError || remainingWaitTime < time.Second {
...
func waitForRateLimit(ctx context.Context, logger Logger, err *slack.RateLimitedError, attempt int, action string, remainingWaitTime time.Duration) error {
...
func waitForTransientError(ctx context.Context, logger Logger, err error, attempt int, action string, remainingWaitTime time.Duration) error {
...
func waitForFatalError(ctx context.Context, logger Logger, err error, attempt int, action string, remainingWaitTime time.Duration) error {
...
func isCtxCanceledErr(args ...any) bool {
	for _, arg := range args {
		...
		case error:
			...
			if strings.Contains(t.Error(), context.Canceled.Error()) {
				return true
			}
		case string:
			if strings.Contains(t, context.Canceled.Error()) {
				return true
			}
		default:
			continue
...
func (c *client) readCacheString(ctx context.Context, cacheKey string) (string, bool, error) {
	cachedValue, cacheHit, err := c.cache.Get(ctx, cacheKey)

	if err == nil {
		return cachedValue, cacheHit, nil
	}

	if !c.options.IgnoreCacheReadErrors {
		return "", false, err
	}
...
func (c *client) readCacheBool(ctx context.Context, cacheKey string) (bool, bool, error) {
	cachedValue, cacheHit, err := c.cache.GetBool(ctx, cacheKey)
...
func (c *client) addToCacheWithRandomExpiration(ctx context.Context, cacheKey string, value any, minExpiration time.Duration, variation time.Duration) {
	if err := c.cache.SetWithRandomExpiration(ctx, cacheKey, value, minExpiration, variation); err != nil && !isCtxCanceledErr(err) {
