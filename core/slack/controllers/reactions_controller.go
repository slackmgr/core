...
func NewReactionsController(eventhandler *handler.SocketModeHandler, client handler.SocketClient, commandHandler common.FifoQueue, cacheStore store.StoreInterface, logger common.Logger, conf *config.Config) *ReactionsController {
...
func (c *ReactionsController) getUserInfo(ctx context.Context, channel, userID string, requireChannelAdmin bool, logger common.Logger) (*slack.User, error) {
	...
	if requireChannelAdmin {
		userIsChannelAdmin := c.conf.Slack.UserIsChannelAdmin(ctx, channel, userInfo.ID, c.client.UserIsInGroup)
