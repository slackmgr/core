...
func (c *Client) Connect(ctx context.Context) error {
	options := &slackapi.Options{
		BotToken:                     c.conf.Slack.BotToken,
		AppToken:                     c.conf.Slack.AppToken,
		CachePrefix:                  c.conf.CachePrefix,
		IgnoreCacheReadErrors:        c.conf.IgnoreCacheReadErrors,
		DebugMode:                    c.conf.Slack.Debug,
		MaxAttemtsForRateLimitError:  c.conf.Slack.MaxAttemtsForRateLimitError,
		MaxAttemptsForTransientError: c.conf.Slack.MaxAttemptsForTransientError,
		MaxAttemptsForFatalError:     c.conf.Slack.MaxAttemptsForFatalError,
		MaxRateLimitErrorWaitTime:    c.conf.Slack.MaxRateLimitErrorWaitTime,
		MaxTransientErrorWaitTime:    c.conf.Slack.MaxTransientErrorWaitTime,
		MaxFatalErrorWaitTime:        c.conf.Slack.MaxFatalErrorWaitTime,
		HTTPTimeout:                  c.conf.Slack.HTTPTimeout,
	}

	c.api = slackapi.New(c.logger, c.cacheStore, c.metrics, options)
...
func (c *Client) RunSocketMode(ctx context.Context) error {
	c.logger.Debug("client.RunSocketMode started")
	defer c.logger.Debug("client.RunSocketMode exited")
...
func (c *Client) Update(ctx context.Context, channelID string, allChannelIssues []*models.Issue) error {
	...
	allowIssueReordering := c.conf.Slack.OrderIssuesBySeverity(channelID) && openIssueCount <= c.conf.ReorderIssueLimit
...
func (c *Client) IsAlertChannel(ctx context.Context, channelID string) (bool, string, error) {
	if c.conf.Slack.IsInfoChannel(channelID) {
	...
	if !botIsInChannel {
		return false, "SUDO Slack Manager is not in channel", nil
	}
...
func getWebhookButtons(issue *models.Issue) []slack.BlockElement {
	...
	for index, hook := range issue.LastAlert.Webhooks {
		if issue.IsResolved() && (hook.DisplayMode == "" || hook.DisplayMode == client.WebhookDisplayModeOpenIssue) {
			continue
		}

		if !issue.IsResolved() && hook.DisplayMode == client.WebhookDisplayModeResolvedIssue {
			continue
		}
